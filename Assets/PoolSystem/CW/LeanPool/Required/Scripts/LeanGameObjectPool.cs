...
namespace Lean.Pool
{
	...
	public class LeanGameObjectPool : ACommonPool, ISerializationCallbackReceiver
	{
		[System.Serializable]
		public class Delay
		{
		...
		{
			if (spawnedClonesHashSet.Contains(clone) == true)
			{
				return true;
			}

			// Search list
			for (var j = spawnedClonesList.Count - 1; j >= 0; j--)
			{
				if (spawnedClonesList[j] == clone)
				{
					return true;
				}
			}
		...
		public override void Despawn(GameObject clone, float t = 0.0f)
		{
			if (clone != null)
			{
				// Delay the despawn?
