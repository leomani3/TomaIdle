...
namespace Lean.Pool
{
	/// <summary>This class handles the association between a spawned prefab, and the ACommonPool component that manages it.</summary>
	public static class LeanPool
	{
		...
		public static Dictionary<GameObject, ACommonPool> Links = new Dictionary<GameObject, ACommonPool>();
		...
		private static GameObject Spawn(GameObject prefab, Vector3 localPosition, Quaternion localRotation, Vector3 localScale, Transform parent, bool worldPositionStays)
		{
			if (prefab != null)
			{
				...
				var pool = default(ACommonPool);

				// Create a new pool for this prefab?
				if (ACommonPool.TryFindPoolByPrefab(prefab, ref pool) == false)
				{
					Debug.LogError("Pool not found for prefab " + prefab.name + ". Creating a new LeanGameObjectPool.");
		...
		{
			foreach (var instance in ACommonPool.CommonInstances)
			{
		...
		public static void Despawn(GameObject clone, float delay = 0.0f)
		{
			if (clone != null)
			{
				...
				{
					if (ACommonPool.TryFindPoolByClone(clone, ref pool) == true)
					{
						pool.Despawn(clone, delay);
					}
					else
